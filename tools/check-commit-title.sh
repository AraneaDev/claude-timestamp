...
#
#   tools/check-commit-title.sh "fix: stop gating on a window that has ended"
#   tools/check-commit-title.sh --file .git/COMMIT_EDITMSG
set -uo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

usage() {
  printf 'usage: check-commit-title.sh <subject> | --file <path>\n' >&2
  exit 2
}
...
case "$1" in
  --file)
    [ -n "${2:-}" ] || usage
    [ -f "$2" ] || {
      printf 'check-commit-title: no such file: %s\n' "$2" >&2
      exit 2
    }
