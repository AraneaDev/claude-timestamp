#!/usr/bin/env bash
# PostToolUse hook -- closes out a tool call started by pre-tool-use.sh.
#
...
input="$(cat)"
read -r session_id tool_use_id tool_name <<< "$(printf '%s' "$input" \
  | jq -r '"\(.session_id // "-") \(.tool_use_id // "") \(.tool_name // "")"')"
